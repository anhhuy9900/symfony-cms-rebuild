...
class AdminSystemModulesValidation extends Controller {
    ...
    public static function loadValidatorMetadata(ClassMetadata $metadata)
    {
        ...
        $metadata->addPropertyConstraint('moduleName', new Assert\NotNull(
            array(
                'message' => 'Module Name is not null',
            )
        ));
        $metadata->addPropertyConstraint('moduleName', new Assert\Length(array(
            'min'        => 6,
            'max'        => 100,
        ...
        /*$metadata->addPropertyConstraint('moduleAlias', new Assert\NotNull(
            array(
                'message' => 'Module Alias is not null',
            )
        ));
        $metadata->addPropertyConstraint('moduleAlias', new Assert\Length(array(
            'min'        => 6,
            'max'        => 100,
        ...
        $metadata->addPropertyConstraint('moduleOrder', new Assert\NotNull(
            array(
                'message' => 'Module Order is not null',
            )
        ));
        $metadata->addPropertyConstraint('moduleOrder', new Assert\Type(
        	  array(
        		    'type'    => 'integer',
